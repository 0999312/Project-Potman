		...
		if (!this.world.isRemote) {
			if (this.getTank() != null) {
				...
				if (isRecipes(tank_fluid, inventoryList)) {
					...
					this.working = RecipesUtil.getInstance().canIncrease(result, itemstack) && isBurning();
					if (working) {
						cookTime += 1;
				...
				if (flag != this.isBurning()) {
					flag1 = true;
			        BlockCampfirePot.setState(this.isBurning(), this.isWorking(), this.world, this.pos);
				}else
